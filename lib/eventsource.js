...
function EventSource (url, eventSourceInitDict) {
  ...
  function connect () {
    ...
    var callback = function (res) {
      ...
      var maximumAllocationSize = 1048576 * 8//8MB
      var sizeUsed = 0;
      ...
      res.on('data', function (chunk) {
        if(!buf) {
          ...
          //allocate new buffer
          if (chunk.length > buf.length - sizeUsed) {
            let sizeToAllocate = (2*(buf.length) + chunk.length)
            if (maximumAllocationSize < sizeToAllocate) {
              sizeToAllocate = maximumAllocationSize + buf.length + chunk.length;
            }
            let newBuffer = Buffer.alloc(sizeToAllocate);
