...
// larger would be indistinguishable from the connection never resuming.
...
function EventSource (url, eventSourceInitDict) {
  ...
  var retryDelayStrategy
  if (config.retryDelayStrategy) {
    retryDelayStrategy = config.retryDelayStrategy
  } else {
    retryDelayStrategy = new retryDelay.RetryDelayStrategy(
  ...
  function receivedEvent (event) {
    // The reset interval measures how long the current connection has been delivering data,
    // so the "good since" time is anchored to the first event of each connection.
  ...
  function parseEventStreamLine (buf, pos, fieldLength, lineLength) {
    if (lineLength === 0) {
      ...
      if (field === 'data') {
        ...
        // The SSE specification only allows this field to contain ASCII digits; any other
        // value is ignored. The accepted value is capped so that a huge reconnection time
        // cannot act as a permanent stop.
        if (/^\d+$/.test(value)) {
